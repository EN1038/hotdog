/**
 * Add sales menu item "กุ้งเสียบ" (10฿) to every STORE branch,
 * same category/channels as other seafood skewers, attach to FROM_MENU promo lists,
 * ensure BrandProduct + stock row for stock UI.
 *
 * Run: node scripts/add-menu-shrimp-skewer-all-branches.mjs
 */
import "dotenv/config";
import { PrismaClient } from "@prisma/client";
import { PrismaPg } from "@prisma/adapter-pg";

const ITEM_NAME = "กุ้งเสียบ";
const PRICE = 10;
const CATEGORY_NAME = "อาหารทะเล";
  ...
  for (const branch of branches) {
    ...
    if (!categoryId) {
      const cat =
        ...
        (await prisma.menuCategory.findFirst({
          where: { branchId: branch.id, name: { contains: "ทะเล" } },
          select: { id: true },
        }));
