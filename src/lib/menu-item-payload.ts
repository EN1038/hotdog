...
export const menuItemCreateSchema = z
  .object({
    ...
    skewerImageUrl: z.string().optional().nullable(),
    quantityUnit: z.string().trim().max(40).optional().nullable(),
    sticksPerUnit: z.number().int().min(1).max(9999).optional(),
    countsAsSticks: z.boolean().optional(),
    skewerMinQty: z.number().int().min(1).max(9999).optional(),
    isHidden: z.boolean().optional(),
    hideFromStaff: z.boolean().optional(),
    isOutOfStock: z.boolean().optional(),
    brandProductId: z.string().nullable().optional(),
