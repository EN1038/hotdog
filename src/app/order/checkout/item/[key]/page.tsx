...
export default function CheckoutItemEditPage() {
  const { key } = useParams<{ key: string }>();
  const decodedKey = decodeURIComponent(key);
  const router = useRouter();
  const { cart, cartBranchId, replaceLine, fulfillment } = useCustomer();
  ...
  const line = useMemo(
    () => cart.find((l) => l.key === decodedKey) ?? null,
    [cart, decodedKey],
  );
  ...
  const item = useMemo(() => {
    if (!branch || !line) return null;
    return branch.menuItems.find((m) => m.id === line.branchMenuItemId) ?? null;
  }, [branch, line]);
  ...
  useEffect(() => {
    ...
    for (const group of item.optionGroups) {
      selected[group.id] = (line.optionIds ?? []).filter((id) =>
              ...
              >
                ...
                <ul className="pb-2">
                  {group.options.map((opt) => {
                    ...
                    const atMax =
                      !isSingle && !active && selectedCount >= group.maxSelect;
                    return (
                      <li key={opt.id}>
                        <button
                          type="button"
                          disabled={atMax}
                          onClick={() =>
                            toggleOption(group.id, opt.id, group.maxSelect)
                          }
