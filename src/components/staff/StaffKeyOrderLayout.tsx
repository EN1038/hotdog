...
/** Bottom nav height in StaffAppShell (keep in sync). */
export const STAFF_BOTTOM_NAV_OFFSET =
  "calc(4.75rem + env(safe-area-inset-bottom, 0px))";

/** Sticky summary sits above the submit footer + bottom nav. */
export const STAFF_KEY_ORDER_STICKY_OFFSET =
  "calc(4.75rem + env(safe-area-inset-bottom, 0px) + 4.75rem)";
...
export function StaffKeyOrderLayout({
  ...
  return (
    <StaffAppShell active="home" showHeader={false}>
      <div
        className={`min-h-[70vh] w-full overflow-x-hidden bg-gray-50 ${
          footer ? "pb-28" : "pb-4"
        }`}
      >
        ...
        {footer ? (
          <div
            className="fixed inset-x-0 z-30 border-t border-gray-200 bg-white/95 px-4 py-3 backdrop-blur"
            style={{ bottom: STAFF_BOTTOM_NAV_OFFSET }}
          >
